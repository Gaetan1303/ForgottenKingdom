...
func _run() -> void:
	...
	Refuge.intro_choice(cm.service_context, "chute_03", choice)
	Refuge.intro_choice(cm.service_context, "chute_03", choice)
	...
	Refuge.repair(cm.service_context, "granary", "pnj_kael")
	check(cm.get_ressource("bois") == wood - 1, "coût réel réparation : soutien Kael +5 appliqué aux matériaux")
	check(Refuge.find_person(cm.service_context, "pnj_kael").etat == "assigne", "vraie affectation")
	check(not Refuge.building_reason(cm.service_context, "walls").is_empty(), "deux décisions interdites dans la même phase")
	Refuge.repair(cm.service_context, "granary", "pnj_kael")
	check(cm.get_ressource("bois") == wood - 1, "réparation non répétable")
	...
	check(Refuge.has(cm.service_context, "combat"), "combat accompli")
	...
	check(Refuge.find_person(cm.service_context, "pnj_kael").etat == "blesse", "blessure persistante")
	cm.sauvegarder()
	cm.charger_sauvegarde()
	check(Refuge.find_person(cm.service_context, "pnj_kael").etat == "blesse", "blessure chargée")
	check(not dungeon.start_expedition(["pnj_kael"]).is_empty(), "blessure empêche départ")
	Refuge.recover(cm.service_context, "pnj_kael")
	check(Refuge.find_person(cm.service_context, "pnj_kael").etat == "disponible", "soins rendent disponible")
	cm.resoudre_planning_pnj_journee()
	cm.advance_day_phase()
	cm.reset_actions_pour_nuit()
	Refuge.repair(cm.service_context, "workshop", "pnj_kael")
	check(Refuge.has(cm.service_context, "rebuild"), "atelier réel restauré")
	check(cm.get_ressource("fer") == 2, "coût atelier")
	var food: int = cm.get_ressource("nourriture")
	Refuge.dawn(cm.service_context)
	...
	check(Refuge.has(cm.service_context, "govern") and not Refuge.has(cm.service_context, "assignment"), "priorité alternative sans réparation imposée")
	cm.campaign.help_mode = 2
	Refuge.social_choice(cm.service_context, false)
	check(Refuge.has(cm.service_context, "social"), "choix social avec aides désactivées")
	Refuge.mark(cm.service_context, "rebuild")
	cm.gagner({"essence": 1})
	var soul_before: int = cm.barre_ame
	var mana_attune: int = cm.get_ressource("mana")
	Refuge.study_relic(cm.service_context, true)
