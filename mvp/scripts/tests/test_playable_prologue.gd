...
func _run() -> void:
	...
	check(cm.get_pnj_gestion_state().roster.size() >= 4, "compagnons canoniques")
	Refuge.initialize(cm)
	check(cm.get_pnj_gestion_state().roster.size() >= 4 and cm.get_ressource("nourriture") == 12, "initialisation idempotente")
	...
	Refuge.intro_choice(cm, "chute_03", choice)
	check(cm.get_ressource("mana") == mana_before + 4, "choix intro sans double gain")
	intro.queue_free()
	await process_frame
	...
	Refuge.repair(cm, "granary", "pnj_sylas")
	check(cm.get_ressource("bois") == wood - 6, "coût réel réparation")
	check(Refuge.find_person(cm, "pnj_sylas").etat == "assigne", "vraie affectation")
	...
	check(not dungeon.start_expedition(["pnj_sylas"]).is_empty(), "compagnon affecté interdit")
	check(not dungeon.start_expedition(["pnj_kael", "pnj_kael"]).is_empty(), "pas de doublon équipe")
	check(dungeon.start_expedition(["pnj_kael", "pnj_miri"]).is_empty(), "départ équipe")
	...
	check(Refuge.find_person(cm, "pnj_miri").etat == "blesse", "blessure persistante")
	cm.sauvegarder()
	cm.charger_sauvegarde()
	check(Refuge.find_person(cm, "pnj_miri").etat == "blesse", "blessure chargée")
	check(not dungeon.start_expedition(["pnj_miri"]).is_empty(), "blessure empêche départ")
	Refuge.recover(cm, "pnj_miri")
	check(Refuge.find_person(cm, "pnj_miri").etat == "disponible", "soins rendent disponible")
	cm.resoudre_planning_pnj_journee()
	cm.moment_journee = "nuit"
	cm.reset_actions_pour_nuit()
	Refuge.repair(cm, "workshop", "pnj_sylas")
	...
	check(cm.barre_ame == soul_before - 5 and cm.get_ressource("mana") == mana_attune + 10, "résonance paie âme et essence")
