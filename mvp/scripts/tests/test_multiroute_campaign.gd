	...
	for route in paths:
		...
		for action in paths[route]:
			...
			while str(action) == "craft_grounded" and cm.campaign.power_routes.artifacts.is_empty() and retries < 10:
				next_decision()
				check(Campaign.execute(cm, "materials").accepted, "récupération après échec")
				Campaign.acknowledge(cm)
				next_decision()
				Campaign.execute(cm, "craft_grounded")
				Campaign.acknowledge(cm)
				retries += 1
		...
		check(Refuge.has(cm, "salvage") and cm.campaign.power_routes.next_objective == "rebuild_refuge", "progression narrative débloquée")
		check(cm.campaign.get("run", {}).is_empty() and dungeon.current_run.is_empty(), "pacifiste : aucune expédition ni combat personnel")
		check(not Refuge.has(cm, "combat"), "aucun accomplissement martial factice")
		cm.sauvegarder()
		...
		check(not Campaign.execute(cm, str(paths[route][-1])).accepted and before == JSON.stringify(cm.get_ressources()), "objectif non répétable après chargement")
		Refuge.repair(cm, "workshop", "pnj_kael")
		check(Refuge.has(cm, "rebuild"), "atelier accessible par " + str(route))
		Refuge.study_relic(cm, false)
		check(Refuge.has(cm, "soul"), "démonstration terminée sans rituel imposé")
		panel.queue_free()
		await settle()
